...
import { INPUT_CLASSES, FREELANCER_AND_TYPE_DATA } from '../../constant';

export default function CreateProjectWithoutLoginPage() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    const { jobType, freelancerType, deadline, projectFee } = formData;

    if (typeof jobType !== 'number' || typeof freelancerType !== 'number' || !projectFee || !deadline) {
        toast.error('Please fill in all required fields.');
        return;
    }

    localStorage.setItem('project-data', JSON.stringify(formData))
    navigate('/login');
    toast.success('Please login to post project.')
        ...
        <form onSubmit={handleSubmit} className='flex flex-col bg-white border rounded-2xl border-2 border-black gap-6 mx-auto mt-4 mb-8'>
          ...
          <div className='p-8 flex flex-col gap-6 '>
            <div className='flex flex-col'>
              <label className='text-left lg:text-xl mb-2'>What type of freelancer are you looking for</label>
              <div className='flex flex-col sm:flex-row justify-between gap-6'>
              { FREELANCER_AND_TYPE_DATA.map((jobType, index) => (
                <div
                  key={jobType.key}
                  className={`${INPUT_CLASSES} w-full sm:w-1/3 h-12 text-center cursor-pointer content-center`}
                  onClick={(e) => handleBoxChanges('jobType', index)}
                  style={{ backgroundColor: formData.jobType === index ? '#A8FFEA' : '' }}
                >
                  {jobType.label}
                </div>
              )) }
              ...
              <>
                <div className='flex flex-col mt-6'>
                  ...
                  <div className='grid grid-cols-1 sm:grid-cols-3 gap-6 mb-6'>
                    { FREELANCER_AND_TYPE_DATA[formData.jobType].children.map((freelancerType, index) => (
                      <div
                        key={freelancerType.key}
                        className={`${INPUT_CLASSES} h-12 text-center cursor-pointer content-center`}
                        style={{ backgroundColor: formData.freelancerType === index ? '#A8FFEA' : '' }}
                        onClick={(e) => handleBoxChanges('freelancerType', index)}
                      >
                        {freelancerType.label}
                      </div>
                    ))}
                  </div>
                </div>
